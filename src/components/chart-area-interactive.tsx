...
export const description = "News views chart"

export type NewsViewsChartPoint = {
  date: string
  views: number
}
...
export function ChartAreaInteractive({ data }: { data: NewsViewsChartPoint[] }) {
  ...
  const totalViews = React.useMemo(
    () => filteredData.reduce((sum, item) => sum + item.views, 0),
    [filteredData]
  )
  ...
  return (
    <Card className="@container/card">
      <CardHeader>
        ...
        <CardDescription>
          {totalViews.toLocaleString()} views in selected range
        </CardDescription>
        <CardAction>
          <ToggleGroup
            type="single"
